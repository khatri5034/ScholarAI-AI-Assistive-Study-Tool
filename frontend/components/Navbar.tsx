...
function ProfileIcon() {
  return (
    <span className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-700 text-slate-300 ring-2 ring-emerald-500/50 transition hover:bg-slate-600 hover:text-white">
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/10 bg-slate-950/80 backdrop-blur-xl">
      <nav className="flex h-16 w-full items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="flex rounded-lg bg-white px-2 py-1.5">
            <Image
        ...
        <div className="hidden flex-1 justify-center px-6 md:flex">
          ...
          <div className="relative w-full max-w-xl">
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-600/80 bg-slate-800/80 py-2 pl-9 pr-4 text-sm text-white placeholder-slate-500 transition focus:border-emerald-500/50 focus:outline-none focus:ring-1 focus:ring-emerald-500/50"
              aria-label="Search"
            />
          </div>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-2 md:gap-4 lg:gap-6">
          ...
          <div className="hidden items-center gap-4 md:flex lg:gap-6">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-sm font-medium text-slate-300 transition hover:text-white whitespace-nowrap"
              >
                {link.label}
              </Link>
            ))}
            {isLoggedIn ? (
              ...
              <>
                <Link
                  href="/login"
                  className="rounded-lg bg-emerald-200 px-4 py-2 text-sm font-medium text-emerald-900 transition hover:bg-emerald-300"
                >
                  Log in
                </Link>
                <Link
                  href="/signup"
                  className="rounded-full bg-emerald-200 px-4 py-2 text-sm font-semibold text-emerald-900 transition hover:bg-emerald-300"
                >
                  Sign up
                </Link>
              </>
          ...
          <button
            type="button"
            className="flex h-11 w-11 flex-col items-center justify-center gap-1.5 rounded-xl border border-slate-600/50 bg-slate-800/50 text-white transition hover:border-emerald-500/50 hover:bg-slate-800 md:hidden"
        ...
        <div className="border-b border-white/10 px-4 py-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              type="search"
              placeholder="Search documents, notes..."
              className="w-full rounded-lg border border-slate-600/80 bg-slate-800/80 py-2.5 pl-9 pr-4 text-sm text-white placeholder-slate-500 focus:border-emerald-500/50 focus:outline-none focus:ring-1 focus:ring-emerald-500/50"
              aria-label="Search"
            />
          </div>
        </div>
          ...
          {isLoggedIn ? (
            ...
            <>
              <li>
                <Link
                  href="/login"
                  className="block rounded-lg bg-emerald-200 px-4 py-3 text-sm font-medium text-emerald-900 transition hover:bg-emerald-300"
                  onClick={() => setMenuOpen(false)}
                >
                  Log in
                </Link>
              ...
              <li>
                <Link
                  href="/signup"
                  className="block rounded-lg bg-emerald-200 px-4 py-3 text-sm font-semibold text-emerald-900 transition hover:bg-emerald-300"
                  onClick={() => setMenuOpen(false)}
                >
                  Sign up
                </Link>
