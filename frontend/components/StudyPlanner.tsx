    ...
    <div className="space-y-6">
      ...
      {plan && (
        <div className="rounded-2xl border border-teal-900/40 bg-gradient-to-b from-slate-900/60 to-slate-950/90 p-6 shadow-lg shadow-black/25">
          ...
          <div className="mt-4">
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Messages</p>
            <div
              className="mt-2 max-h-[min(360px,45vh)] space-y-3 overflow-y-auto rounded-xl border border-slate-800/90 bg-slate-950/60 p-3 ring-1 ring-black/30"
              role="log"
              aria-label="Plan Q and A conversation"
            >
              {planChatMessages.length === 0 ? (
                <p className="py-6 text-center text-sm text-slate-600">Nothing here yet—ask something blunt.</p>
              ) : (
                planChatMessages.map((m, i) => (
                  <div key={i} className={m.role === "user" ? "flex justify-end" : "flex justify-start"}>
                    ...
                    >
                      {m.role === "assistant" ? renderPlanWithBold(m.content) : m.content}
                    </div>
                  </div>
                ))
              )}
